...
interface IRestaurantsDropdownProps {
  value: boolean | any;
  onChange: any;
}

const RestaurantsDropdown: React.FunctionComponent<IRestaurantsDropdownProps> = ({
  value,
  onChange,
}) => {
    ...
    <div className="relative inline-block w-full">
      <div>
        <span className="rounded-md shadow-sm">
          ...
          >
            {value ? value.name : "Choose restaurant"}
              ...
              items.map((item, index) => (
                ...
                >
                  ...
                  <div className="flex space-x-2">
                    <span className="px-2.5 py-0.5 rounded-full text-xs font-medium leading-4 bg-cool-gray-100 text-cool-gray-800 capitalize">
                      {item.id}
                    </span>
                  </div>
                </div>
